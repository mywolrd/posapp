...
public class JdbcCustomerDao extends JdbcBaseDao implements CustomerDao {
    ...
    private final static String listByLastName = "SELECT * from CUSTOMERS where UPPER(CUSTOMERS.lastName) like :lastName";
    ...
    public long save(CustomerParameter info) {
        SqlParameterSource parameter = new MapSqlParameterSource()
                .addValue(DBNames.LASTNAME, info.getLastName())
                .addValue(DBNames.FIRSTNAME, info.getFirstName())
        ...
        try {
            ...
            if (!holder.getKeyList().isEmpty()) {
                Integer id = (Integer) holder.getKeyList().get(0)
                        .get(DBNames.ID);
                return id;
            }
    ...
    public List<Customer> searchByLikeLastName(String querystr) {
        SqlParameterSource parameter = new MapSqlParameterSource()
                .addValue(DBNames.LASTNAME, querystr.toUpperCase() + "%");
        try {
            return this.namedParameterJdbcTemplate.query(listByLastName,
                    parameter, rowMapper);
        } catch (DataAccessException e) {
    ...
    public Customer uniqueById(long id) {
        ...
        try {
            return this.namedParameterJdbcTemplate.queryForObject(uniqueById,
                    parameter, rowMapper);
        } catch (DataAccessException e) {
            return null;
        }
